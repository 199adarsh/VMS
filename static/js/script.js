...
function openEditUserModal(user) {
  ...
  if (Object.keys(updateData).length > 0) {
    ...
    showMessage(
      allUsersList.parentElement.querySelector(".message-box") || allUsersList,
      "No changes detected.",
      "error"
    );
  }
...
function openEditTaskModal(task) {
  ...
  if (Object.keys(updateData).length > 0) {
    ...
    showMessage(
      allTasksList.parentElement.querySelector(".message-box") || allTasksList,
      "No changes detected.",
      "error"
    );
  }
...
async function updateDashboardWidgets() {
  try {
    ...
    // Tasks
    document.getElementById("dashboard-tasks-count").textContent = summary.tasks
